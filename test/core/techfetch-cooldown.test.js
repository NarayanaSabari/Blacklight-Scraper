...
test('cooldownPath: ends with .blacklight-techfetch-cooldown in the homedir', () => {
    assert.ok(cooldownPath().endsWith('.blacklight-techfetch-cooldown'), cooldownPath());
});
...
test('cooldownMs: reads positive integer env TECHFETCH_BLOCK_COOLDOWN_MIN', () => {
    assert.equal(cooldownMs({ TECHFETCH_BLOCK_COOLDOWN_MIN: '15' }), 15 * 60 * 1000);
    assert.equal(cooldownMs({ TECHFETCH_BLOCK_COOLDOWN_MIN: '120' }), 120 * 60 * 1000);
});

test('cooldownMs: ignores zero / negative / garbage env values (60-min default)', () => {
    assert.equal(cooldownMs({ TECHFETCH_BLOCK_COOLDOWN_MIN: '0' }), 60 * 60 * 1000);
    assert.equal(cooldownMs({ TECHFETCH_BLOCK_COOLDOWN_MIN: '-5' }), 60 * 60 * 1000);
    assert.equal(cooldownMs({ TECHFETCH_BLOCK_COOLDOWN_MIN: 'abc' }), 60 * 60 * 1000);
    assert.equal(cooldownMs({ TECHFETCH_BLOCK_COOLDOWN_MIN: '' }), 60 * 60 * 1000);
});
...
test('isOnCooldown: null → false, future → true, past/equal → false', () => {
