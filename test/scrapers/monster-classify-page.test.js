...
test('classifyMonsterPage: empty-payload (suppress) + "no jobs found" text → soft_blocked (masking-bug fix)', () => {
    const r = classifyMonsterPage({
        url: 'https://www.monster.com/jobs/search?q=devops+engineer&page=1',
        bodyText: 'No jobs found matching your search. Try broadening it.',
        cardCount: 0,
        sawApiResponse: true,
        apiResponseInspection: 'empty-payload', // totalSize>0 but empty array = DataDome suppress
    });
    assert.equal(r.state, 'soft_blocked'); // NOT empty_confirmed — the page text is faked
});

test('classifyMonsterPage: empty-results (genuine 0, totalSize 0) → empty_confirmed', () => {
    const r = classifyMonsterPage({
        url: 'https://www.monster.com/jobs/search?q=unobtainium-welder&page=1',
        bodyText: 'No jobs found matching your search.',
        cardCount: 0,
        sawApiResponse: true,
        apiResponseInspection: 'empty-results',
    });
    assert.equal(r.state, 'empty_confirmed');
});

test('classifyMonsterPage: unparseable appsapi (403 captcha body) + 0 cards → soft_blocked', () => {
    const r = classifyMonsterPage({
        url: 'https://www.monster.com/jobs/search?q=devops&page=1',
        bodyText: 'No jobs found ...',
        cardCount: 0,
        sawApiResponse: true,
        apiResponseInspection: 'unparseable',
    });
    assert.equal(r.state, 'soft_blocked');
});
